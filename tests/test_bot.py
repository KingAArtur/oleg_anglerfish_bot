...
async def test_command_horoscope(bot, user):
    ...
    expected_reply_text = (
        'DrillerName, давай воздержимся от бухла хотя бы сегодня!\n'
        'Тебя постоянно будут отвлекать от работы, нужно проявить терпение!\n'
        'Кто-то очень захочет жахнуть тебя сегодня. Отнесись с пониманием!\n'
        'Сегодня ты очень захочешь жахнуться с кем-нибудь, но даже не пытайся!\n'
        'Сегодня ты назовешь кого-то чужим именем.\n\n'
        'Какой-нибудь котик захочет манипулировать тобой, чтобы его накормили. Не поддавайся!\n'
    )
